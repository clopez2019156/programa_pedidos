...
$primera_consulta = "SELECT ROUND(SUM(total), 2) AS total FROM pedidounhesa WHERE idEstado = 2 AND fecha_emision BETWEEN CURRENT_DATE() AND DATE_SUB(CURDATE(), INTERVAL -1 DAY) AND YEAR(fecha_emision) = YEAR(CURRENT_DATE()) AND idVendedor =" . $_SESSION["idUsuario"];
...
$tercera_consulta = "SELECT SUM(monto) AS Meta FROM meta m INNER JOIN usuario u ON m.idUsuario = u.idUsuario WHERE u.idempresa = 1 AND( NOW() BETWEEN m.fecha_inicio AND m.fecha_final) AND u.idUsuario = " . $_SESSION["idUsuario"];
...
// Porcentaje Meta
$quinta_consulta = "SELECT ROUND( ( ( SELECT SUM(total) AS Vendido FROM pedidounhesa WHERE MONTH(fecha_emision) = MONTH(CURRENT_DATE()) AND idEstado = 2 AND idVendedor = " . $_SESSION["idUsuario"] . ") * 100 ) /( SELECT SUM(monto) AS Meta FROM meta m INNER JOIN usuario u ON m.idUsuario = u.idUsuario WHERE u.idempresa = 1 AND u.idUsuario = " . $_SESSION["idUsuario"] . " AND( NOW() BETWEEN m.fecha_inicio AND m.fecha_final) ), 2 ) AS 'Porcentaje'";
